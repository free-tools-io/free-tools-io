    ...
    return new Promise((resolve, reject) => {
        ...
        if(validation.passes()) {
            if(req.query.action === 'Encrypt') {
                var data = CryptoJS.AES.encrypt(req.query.string, req.query.pw);
                data = data.toString()

                return resolve({
                    data: data,
                    status: "success",
                    title: "AES Encrypt for '" + req.query.string.substring(0,10) + "...'"
                });
            } else if (req.query.action === 'Decrypt') {
                var decr = CryptoJS.AES.decrypt(req.query.string, req.query.pw);
                decr = decr.toString(CryptoJS.enc.Utf8);

                return resolve({
                    data: decr,
                    status: "success",
                    title: "AES Decrypt for '" + req.query.string.substring(0,10) + "...'"
                });
            }
        } else {

    ...
    return new Promise((resolve, reject) => {
        ...
        if(validation.passes()) {
            if(req.query.action === 'Encrypt') {
                var data = CryptoJS.RC4Drop.encrypt(req.query.string, req.query.pw);
                data = data.toString()

                return resolve({
                    data: data,
                    status: "success",
                    title: "RC4Drop Encrypt for '" + req.query.string.substring(0,10) + "...'"
                });
            } else if (req.query.action === 'Decrypt') {
                var decr = CryptoJS.RC4Drop.decrypt(req.query.string, req.query.pw);
                decr = decr.toString(CryptoJS.enc.Utf8);

                return resolve({
                    data: decr,
                    status: "success",
                    title: "RC4Drop Decrypt for '" + req.query.string.substring(0,10) + "...'"
                });
            }
        } else {
